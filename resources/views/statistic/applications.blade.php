    ...
    <div class="container">
        <div class="card">
            <div class="card-header">
                ...
                <p class="float-right" ><b>{{ $date }}</b></p></div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                            ...
                            <table class="table">
                                ...
                                <tr>
                                    ...
                                    @foreach($criteria as $item)
                                        <th>{{ $item->name }} ( {{ $item->ball }} ball )</th>
                                    @endforeach
                                    <th>Jami</th>
                                </tr>
                                </thead>
                                ...
                                @foreach($applications as $item)
                                    <tr>
                                        ...
                                        @foreach($criteria as $data)
                                            @if (!isset($scores[$item->id][$data->id]))
                                                <td><b>0</b></td>
                                            @else
                                                <td>
                                                        <?php $sum += number_format($scores[$item->id][$data->id] / $count[$item->id][$data->id], 1); ?>
                                                    <b>{{ number_format($scores[$item->id][$data->id] / $count[$item->id][$data->id], 1) }}</b>
                                                </td>
                                            @endif
                                        @endforeach
                                        <td><b>{{ $sum }}</b></td>
...
<script>
    ...
    setTimeout(refreshPage, 180000); // 180000 milliseconds = 3 minutes
</script>
</body>
</html>
